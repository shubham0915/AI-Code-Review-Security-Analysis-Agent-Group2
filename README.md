...
| **Static Analysis Engine** | **Basic AST Scanning:** Relied on Bandit for Python, which evaluates code line-by-line and lacks deep inter-procedural taint tracking, often missing complex injection attacks. | **Semgrep Structural Taint Tracking:** Upgraded the Python pipeline to execute concurrent Semgrep scans using the `p/python` and `p/secrets` rulepacks, enabling enterprise-grade taint-flow analysis. | **DAST Integration:** Implementing Dynamic Application Security Testing (DAST) to evaluate running application state alongside static structural reviews. |
| **Frontend UI State** | **Naive String Heuristics:** The React dashboard determined the "APPROVED/BLOCKED" state by simply checking if the `overallRisk` string was "CRITICAL" or "HIGH". | **Backend Gatekeeper Synchronization:** Hard-bound the UI state directly to the PR Summary Agent's strict `approved: boolean` flag, ensuring total consistency between the backend scorecard and the UI. | **Granular Rule Configurations:** Allowing developers to configure their own custom risk tolerance rules via the UI instead of a hardcoded backend boolean. |
